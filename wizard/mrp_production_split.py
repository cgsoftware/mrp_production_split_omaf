...
<form string="Split Wizard" col="2">
	...
	<label string="Questo Processo Suddivide in Due Un Ordine di Produzione" colspan="2"/>
	<label string="" colspan="2"/>	             
	<label string= "Specificare la Quantita del Nuovo Ordine di Produzione" colspan="2"/>
    <field name="quantity"/>
</form>
"""
...
class mrp_production_split(wizard.interface):
    def _split(self, cr, uid, data, context):
        ...
        id = data.get('id')
        if not id:
            raise wizard.except_wizard(_('Error !'), _('Devi Selezionare un Ordine di Produzione !'))
        quantity = data['form']['quantity']
        if not quantity or quantity <= 0:
            raise wizard.except_wizard(_('Error !'), _('Devi indicare una qta maggiore di  0.'))
        productions = pool.get('mrp.production')._split(cr, uid, id, quantity, context)
        return {
            'domain': "[('id','in',%s)]" % productions,
            'name': _('Ordini di Produzione'),
            'view_type': 'form',
            'view_mode': 'tree,form',
